...
public class GuiMachineHumaine extends GuiContainer {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
        Reference.MOD_ID, "textures/gui/gui_diarh33.png");
    ...
    protected void drawGuiContainerBackgroundLayer(float pt, int mx, int my) {
        ...
        // === DYNAMIC BAR FILLS ===
        int foodCount = tile.getStackInSlot(0).isEmpty() ? 0 : tile.getStackInSlot(0).getCount();
        fillBar(x + 34, y + 18, 14, 92, foodCount, 64, 0xFF307830, 0xFF50CC50);
        fillBar(x + 56, y + 18, 14, 92, tile.getField(1),
            TileMachineHumaine.TANK_CAPACITY, 0xFF2870B8, 0xFF44CCFF);
        fillBar(x + 78, y + 18, 14, 92, tile.getField(0),
            TileMachineHumaine.RF_CAPACITY, 0xFFCC4444, 0xFFFF6666);
        fillBar(x + 154, y + 18, 18, 92, tile.getField(2),
            TileMachineHumaine.TANK_CAPACITY, 0xFF6B4513, 0xFFA06820);

        // === PROGRESS FILL ===
        int prog = tile.getField(3);
        int maxP = TileMachineHumaine.PROCESS_TIME;
        if (maxP > 0 && prog > 0) {
            int fillW = (int)(46.0F * prog / maxP);
            drawRect(x + 100, y + 34, x + 100 + fillW, y + 76, 0xFF3A1F5E);
        }

        // === CONFIG TAB (drawn from texture u=200,v=0) ===
        mc.getTextureManager().bindTexture(TEXTURE);
    ...
    protected void drawGuiContainerForegroundLayer(int mx, int my) {
        ...
        fontRenderer.drawStringWithShadow(title, (xSize - tw) / 2.0F, 4, 0xDD88FF);

        // Bar letters (inside top of each bar)
        fontRenderer.drawStringWithShadow("F", 37, 20, 0xFF50CC50);
        fontRenderer.drawStringWithShadow("W", 59, 20, 0xFF44CCFF);
        fontRenderer.drawStringWithShadow("E", 81, 20, 0xFFFF6666);
        fontRenderer.drawStringWithShadow("S", 159, 20, 0xFFA06820);
        ...
        fontRenderer.drawStringWithShadow(pct, 123 - pw / 2.0F, 51, 0xDD88FF);

        // Arrows
        fontRenderer.drawStringWithShadow("\u00bb", 93, 51, 0xDD88FF);
        fontRenderer.drawStringWithShadow("\u00bb", 148, 51, 0xDD88FF);

        // Labels
        fontRenderer.drawStringWithShadow("Digestion", 103, 79, 0xFF8866AA);
        fontRenderer.drawStringWithShadow(
            TileMachineHumaine.RF_PER_TICK + " Bio-E/t", 8, 118, 0xFF8866AA);
    ...
    private void drawBarTooltips(int mx, int my) {
        int x = guiLeft;
        int y = guiTop;
        if (inRect(mx, my, x+34, y+18, 14, 92))
            drawHoveringText(Collections.singletonList(
                "Food: " + (tile.getStackInSlot(0).isEmpty()
                    ? "Vide" : tile.getStackInSlot(0).getDisplayName())), mx, my);
        if (inRect(mx, my, x+56, y+18, 14, 92))
            drawHoveringText(Collections.singletonList(
                tile.getField(1) + "/" + TileMachineHumaine.TANK_CAPACITY + " mB H2O"), mx, my);
        if (inRect(mx, my, x+78, y+18, 14, 92))
            drawHoveringText(Collections.singletonList(
                tile.getField(0) + "/" + TileMachineHumaine.RF_CAPACITY + " Bio-E"), mx, my);
        if (inRect(mx, my, x+154, y+18, 18, 92))
            drawHoveringText(Collections.singletonList(
                tile.getField(2) + "/" + TileMachineHumaine.TANK_CAPACITY + " mB Sortie"), mx, my);
    }
    ...
    private void drawConfigPanel(int mx, int my) {
        ...
        String t = "Side Config - " + TYPE_NAMES[configType];
        fontRenderer.drawStringWithShadow(t, px+6, py+4, 0xFFDD88FF);
        ...
        // Face tooltips (layout aligne sur four : SOUTH centre, NORTH bas-gauche, DOWN bas-centre)
        int[][] btns = {
