...
public class TESRCondenseur extends TileEntitySpecialRenderer<TileCondenseur> {
    ...
    private void renderArms(TileCondenseur te, double x, double y, double z,
        ...
        // Bottom layer blocks (y=0 relative to master):
        //   Master(condenseur/hole) at (0,0)
        //   Block at (dx, 0)  - wall or redstone
        //   Block at (0, dz)  - wall or redstone
        //   Block at (dx, dz) - wall or redstone
        // Arms are anchored on the TOP FACE of these 3 non-master blocks,
        // on the inner edge (facing the center), extending up into the glass area

        // Arm anchor Y: top of bottom blocks = y+1.0, arms sit just above
        float armY = 1.05F;
        ...
        // Arm 1 on block (dx, 0) - compacts handler (slots 0,1)
        // Anchor: center of that block, inner edge facing master
        float a1x = dx + 0.5F;
        float a1z = 0.5F;
        ...
        // Arm 2 on block (0, dz) - catalyst handler (slot 3)
        float a2x = 0.5F;
        float a2z = dz + 0.5F;
        ...
        // Arm 3 on block (dx, dz) - key handler (slot 2)
        float a3x = dx + 0.5F;
        float a3z = dz + 0.5F;
    ...
    private void drawArm(float anchorX, float anchorY, float anchorZ,
        ...
        // Base mount on top of the block
        drawBox(-0.09F, -0.05F, -0.09F, 0.09F, 0.05F, 0.09F,
                0.18F*brightness, 0.18F*brightness, 0.22F*brightness, 1.0F);

        // Vertical piston going up into glass area
        drawBox(-0.04F, 0.05F, -0.04F, 0.04F, 0.30F, 0.04F,
                bR*0.8F, bG*0.8F, bB*0.8F, 1.0F);

        // Shoulder joint at top of vertical part
        GlStateManager.translate(0, 0.30F, 0);
        drawBox(-0.07F, -0.03F, -0.07F, 0.07F, 0.03F, 0.07F,
                jR, jG, jB, 1.0F);

        // Rotate horizontally to face center + swing
        GlStateManager.rotate(-yawAngle + swingOffset, 0, 1, 0);

        // Horizontal upper arm toward center
        drawBox(-armW, -armH, 0, armW, armH, seg1,
                bR, bG, bB, 1.0F);

        // Elbow joint
        GlStateManager.translate(0, 0, seg1);
        drawBox(-0.065F, -0.065F, -0.025F, 0.065F, 0.065F, 0.025F,
                jR, jG, jB, 1.0F);

        // Slight downward bend at elbow
        float elbowBend = 10.0F + 5.0F * (float)Math.sin(time * 1.0);
        ...
        drawBox(-clawOpen-0.02F, -0.025F, 0, -clawOpen+0.005F, 0.025F, 0.06F,
                cR, cG, cB, 1.0F);
        drawBox(clawOpen-0.005F, -0.025F, 0, clawOpen+0.02F, 0.025F, 0.06F,
                cR, cG, cB, 1.0F);
    ...
    private void renderFluid(TileCondenseur te, double x, double y, double z,
        ...
        bf.pos(mnX,tY+w2,mxZ).color(r,g,b,a).endVertex(); ts.draw();
        bf.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
        ...
        bf.pos(mnX,tY+w1,mnZ).color(r,g,b,sa).endVertex(); ts.draw();
        bf.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
        ...
        bf.pos(mxX,tY+w1,mxZ).color(r,g,b,sa).endVertex(); ts.draw();
        bf.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
        ...
        bf.pos(mnX,tY+w2,mxZ).color(r,g,b,sa).endVertex(); ts.draw();
        bf.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
        ...
        bf.pos(mxX,tY+w2,mnZ).color(r,g,b,sa).endVertex(); ts.draw();
        bf.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
        ...
        bf.pos(mxX,fY,mnZ).color(r*.5F,g*.5F,b*.5F,sa).endVertex(); ts.draw();
