package com.nexusabsolu.mod.blocks.machines;

import com.nexusabsolu.mod.NexusAbsoluMod;
import com.nexusabsolu.mod.Reference;
import com.nexusabsolu.mod.tiles.TileItemInput;
import com.nexusabsolu.mod.util.IHasModel;
import com.nexusabsolu.mod.init.ModBlocks;
import com.nexusabsolu.mod.init.ModItems;
import net.minecraft.block.Block;
import net.minecraft.block.material.Material;
import net.minecraft.block.state.IBlockState;
import net.minecraft.entity.player.EntityPlayer;
import net.minecraft.inventory.InventoryHelper;
import net.minecraft.item.Item;
import net.minecraft.item.ItemBlock;
import net.minecraft.tileentity.TileEntity;
import net.minecraft.util.EnumFacing;
import net.minecraft.util.EnumHand;
import net.minecraft.util.math.BlockPos;
import net.minecraft.world.World;

public class BlockItemInput extends Block implements IHasModel {

    public BlockItemInput() {
        super(Material.IRON);
        setUnlocalizedName("item_input");
        setRegistryName(Reference.MOD_ID, "item_input");
        setHardness(5.0F);
        setResistance(10.0F);
        setCreativeTab(NexusAbsoluMod.nexusTab);
        ModBlocks.BLOCKS.add(this);
        ModItems.ITEMS.add(new ItemBlock(this).setRegistryName(getRegistryName()));
    }

    @Override
    public boolean hasTileEntity(IBlockState state) {
        return true;
    }

    @Override
    public TileEntity createTileEntity(World world, IBlockState state) {
        return new TileItemInput();
    }

    @Override
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state,
                                    EntityPlayer player, EnumHand hand,
                                    EnumFacing facing, float hitX, float hitY, float hitZ) {
        // Find the master block and open its GUI instead
        if (!world.isRemote) {
            TileEntity te = world.getTileEntity(pos);
            if (te instanceof TileItemInput) {
                BlockPos masterPos = ((TileItemInput) te).getMasterPos();
                if (masterPos != null) {
                    TileEntity masterTe = world.getTileEntity(masterPos);
                    if (masterTe instanceof com.nexusabsolu.mod.tiles.TileCondenseurT2) {
                        player.openGui(NexusAbsoluMod.instance, 3, world,
                            masterPos.getX(), masterPos.getY(), masterPos.getZ());
                        return true;
                    }
                }
            }
        }
        return true;
    }

    @Override
    public void breakBlock(World world, BlockPos pos, IBlockState state) {
        TileEntity te = world.getTileEntity(pos);
        if (te instanceof TileItemInput) {
            TileItemInput input = (TileItemInput) te;
            for (int i = 0; i < input.getSizeInventory(); i++) {
                if (!input.getStackInSlot(i).isEmpty()) {
                    InventoryHelper.spawnItemStack(world, pos.getX(), pos.getY(), pos.getZ(),
                        input.getStackInSlot(i));
                }
            }
        }
        super.breakBlock(world, pos, state);
    }

    @Override
    public void registerModels() {
        NexusAbsoluMod.proxy.registerItemRenderer(
            Item.getItemFromBlock(this), 0, "item_input");
    }
}
