...
# Generate inheritance map from forge jar
java -jar "$SPECIALSOURCE" -H build/inheritance.map --in-jar "$FORGE" 2>&1
# Remap using inheritance map
java -jar "$SPECIALSOURCE" --in-jar build/libs/NexusAbsolu-1.0.0-dev.jar --out-jar build/libs/NexusAbsolu-1.0.0.jar --srg-in "$SRG" -h build/inheritance.map 2>&1
if [ $? -ne 0 ]; then
    echo "REOBF FAILED!"
    exit 1
fi
