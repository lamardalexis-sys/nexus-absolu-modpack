...
# Modular Machinery : necessaire pour compiler le moteur de machines Nexus
# (MachineRegistry, DynamicMachine, TileMachineController...). Detecte par
# glob pour survivre a un bump de version du jar dans le modpack.
MODULARMACHINERY=$(find "$MODS_DIR" -maxdepth 1 -iname "modularmachinery-*.jar" 2>/dev/null | head -1)
if [ -z "$MODULARMACHINERY" ]; then
    echo "ERROR: modularmachinery jar introuvable dans $MODS_DIR"
    echo "Attendu : modularmachinery-1.12.2-1.11.1.jar"
    exit 1
fi
echo "Modular Machinery : $(basename "$MODULARMACHINERY")"
